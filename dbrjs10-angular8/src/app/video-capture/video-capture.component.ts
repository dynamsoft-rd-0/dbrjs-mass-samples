...
const { CameraEnhancer, CameraView } = (self as any).Dynamsoft.DCE;
const { CaptureVisionRouter } = (self as any).Dynamsoft.CVR;
const { MultiFrameResultCrossFilter } = (self as any).Dynamsoft.Utility;
...
export class VideoCaptureComponent {
  @ViewChild('cameraViewContainer', {static:true}) cameraViewContainer?: ElementRef<HTMLDivElement>;
  ...
  async ngAfterViewInit(): Promise<void> {
    try {
      ...
      if (ex && (ex as Error).message === componentDestroyedErrorMsg) {
  ...
  async ngOnDestroy() {
    ...
    try {
      ...
      await this.pInit;
      this.cvRouter && this.cvRouter.dispose();
      this.cameraEnhancer && this.cameraEnhancer.dispose();
    } catch (_) {}
  }
}
