...
function VideoDecoderComponent(){

    let [pScanner, setPScanner] = useState(null as null | Promise<BarcodeScanner>);
    const refUiElement = useRef(null as null | HTMLDivElement);

    useEffect(()=>{
        // create instance
        setPScanner(pScanner = BarcodeScanner.createInstance());
        // register how to destory
        return ()=>{(async()=>{
            if(pScanner){
                const scanner = await pScanner;
                scanner.destroyContext();
                (scanner as any).dce&&(scanner as any).dce.dispose(false); // a temp bug fix, will fix in 9.4.0
            }
        })()};
    },[]);

    useEffect(()=>{
        if(refUiElement.current){
            (async()=>{
                const scanner = (await pScanner)!;
                // register uiElement
                if(!scanner.isContextDestroyed())await scanner.setUIElement(refUiElement.current!);
                /**
                 * If you have other settings, write here.
                 */
                if(!scanner.isContextDestroyed())await scanner.open();
            })();
        }
    }, [!!refUiElement.current]);

    return (<div ref={refUiElement} style={{position:"relative",width:"100%",height:"100%",minWidth:"100px",minHeight:"100px",background:"#ddd"}}>
        ...
        <div className="dbr-msg-poweredby" style={{position:"absolute",left:"50%",bottom:"10%",transform:"translateX(-50%)",pointerEvents:"none"}}>
